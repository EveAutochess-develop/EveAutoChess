...
const ROOT_NAME := "EngineTrails"
const FALLBACK_SIDES := 8
const FALLBACK_RADIUS := 0.11
const THICKNESS_FRAC := 0.16
const DEFAULT_STAMP_INTERVAL_S := 1.0
const DEFAULT_UNMANNED_STAMP_INTERVAL_S := 0.35
const DEFAULT_MIN_STAMP_WU := 0.35
const DEFAULT_MAX_SEGMENTS := 6
const DEFAULT_FADE_POWER := 2.0
const MIN_SEGMENT_WU := 0.12
const STYLE_TUBE := "tube"
const STYLE_RIBBON := "ribbon"
...
static func ensure_on(unit: Node3D, team_player: bool) -> EngineBoosterTrail:
	_purge_legacy(unit)
	var trail := unit.get_node_or_null(ROOT_NAME) as EngineBoosterTrail
	if trail == null:
...
static func _purge_legacy(unit: Node3D) -> void:
	var legacy := unit.get_node_or_null("EngineTrail")
	if legacy:
		legacy.name = "EngineTrail_legacy"
		legacy.queue_free()
	var root := unit.get_node_or_null(ROOT_NAME)
...
func setup(unit: Node3D, team_player: bool) -> void:
	...
	_is_unmanned = bool(unit.get("is_unmanned")) if unit else false
...
func _apply_configured_mesh_style() -> void:
	...
	var raw := str(_vis().get("booster_trail_mesh_style", STYLE_RIBBON)).strip_edges().to_lower()
	var next := STYLE_RIBBON if raw != STYLE_TUBE else STYLE_TUBE
	if next == _mesh_style:
		return
	_mesh_style = next
	clear_wake()
...
func set_mesh_style(style: String) -> void:
	...
	var next := STYLE_RIBBON if style == STYLE_RIBBON else STYLE_TUBE
	if next == _mesh_style:
		return
	_mesh_style = next
	clear_wake()
...
func _stamp_interval() -> float:
	...
	if _is_unmanned:
		return maxf(0.05, float(_vis().get("booster_trail_unmanned_stamp_interval_s", DEFAULT_UNMANNED_STAMP_INTERVAL_S)))
	return maxf(0.05, float(_vis().get("booster_trail_stamp_interval_s", DEFAULT_STAMP_INTERVAL_S)))


func _min_stamp_wu() -> float:
	return maxf(0.05, float(_vis().get("booster_trail_min_stamp_wu", DEFAULT_MIN_STAMP_WU)))


func _max_segments() -> int:
	if _max_segments_override > 0:
		return _max_segments_override
	return maxi(1, int(_vis().get("booster_trail_max_segments", DEFAULT_MAX_SEGMENTS)))


func _fade_power() -> float:
	return maxf(1.0, float(_vis().get("booster_trail_fade_power", DEFAULT_FADE_POWER)))


func _lifetime_s() -> float:
	if _lifetime_override_s > 0.05:
		return _lifetime_override_s
	var life := float(_vis().get("booster_trail_lifetime_s", 2.0))
	if life > 0.05:
		return life
...
func _process(delta: float) -> void:
	...
	for h in _histories:
		...
		for i in range(arr.size() - 1, -1, -1):
			...
			arr[i] = s
			if float(s["age"]) >= max_age:
				arr.remove_at(i)
	if _emitting:
		_stamp_acc += delta
		var interval := _stamp_interval()
...
func _live_nozzle_world_outline(i: int) -> PackedVector3Array:
	var local_outline := _nozzle_local_outline(i)
	var world_outer := PackedVector3Array()
	if _ship == null or local_outline.is_empty():
		return world_outer
	for lp in local_outline:
		if _ship.has_method("visual_to_global"):
			world_outer.append(_ship.call("visual_to_global", lp) as Vector3)
		else:
...
func _live_nozzle_center_radius(i: int) -> Dictionary:
	...
	var world_c: Vector3
	if _ship.has_method("visual_to_global"):
		world_c = _ship.call("visual_to_global", local_c) as Vector3
	else:
		world_c = _ship.to_global(local_c)
	var r := FALLBACK_RADIUS
	if i < _radii.size():
		r = maxf(_radii[i], 0.04)
	## Scale radius by the drawn mesh (visual soft-follow / CG long-axis).
	var sc := _ship.global_transform.basis.get_scale()
...
func _stamp_rings() -> void:
	if _ship == null:
		return
	var cap := _max_segments()
	var min_wu := _min_stamp_wu()
	for i in range(_locals.size()):
		...
		if _mesh_style == STYLE_RIBBON:
			...
			if sample.is_empty():
				continue
			center = sample["center"] as Vector3
		else:
			var world_outer := _live_nozzle_world_outline(i)
		...
		if _mesh_style == STYLE_RIBBON:
			(_histories[i] as Array).push_front({
				"center": center,
				"radius": float(sample["radius"]),
				"age": 0.0,
			})
		else:
			(_histories[i] as Array).push_front(sample)
		while (_histories[i] as Array).size() > cap:
			(_histories[i] as Array).pop_back()
...
func _rebuild_ribbons() -> void:
	...
	for i in range(_meshes.size()):
		...
		while _histories.size() <= i:
			_histories.append([])
		var wake: Array = _histories[i]
		var live := _live_nozzle_center_radius(i)
		var samples: Array = []
...
func _idle_plume_ribbon_sample(live: Dictionary) -> Dictionary:
	var back := (_ship.global_transform.basis * _astern_local).normalized() * _idle_plume_wu
	return {
		"center": (live["center"] as Vector3) + back,
		"radius": float(live["radius"]) * 0.85,
...
func _build_crossed_ribbon(samples: Array, tint: Color) -> ArrayMesh:
	...
	var bright := clampf(float(_vis().get("booster_trail_brightness", 0.5)), 0.0, 2.0)
	var centers: Array[Vector3] = []
	var radii: Array[float] = []
	var colors: Array[Color] = []
	for s in samples:
		centers.append(s["center"] as Vector3)
		radii.append(maxf(float(s.get("radius", FALLBACK_RADIUS)), 0.04))
		var a := tint.a * _fade_alpha(float(s.get("age", 0.0))) * bright
	...
	for i in range(centers.size() - 1):
		...
		if absf(dir.dot(up)) > 0.92:
			up = Vector3.RIGHT
		var side_a := dir.cross(up).normalized()
		var side_b := dir.cross(side_a).normalized()
...
func _rebuild_tubes() -> void:
	...
	for i in range(_meshes.size()):
		...
		while _histories.size() <= i:
			_histories.append([])
		var wake: Array = _histories[i]
		var live_outer := _live_nozzle_world_outline(i)
		var samples: Array = []
		...
		if samples.size() < 2:
			if samples.size() == 1:
				mi.mesh = _build_ring_outline(samples[0], _tint)
			else:
				mi.mesh = null
			continue
...
func _sample_color(sample: Dictionary, tint: Color) -> Color:
	var bright := clampf(float(_vis().get("booster_trail_brightness", 0.5)), 0.0, 2.0)
	var a := tint.a * _fade_alpha(float(sample.get("age", 0.0))) * bright
	return Color(tint.r, tint.g, tint.b, a)


func _build_ring_outline(sample: Dictionary, tint: Color) -> ArrayMesh:
	var col := _sample_color(sample, tint)
	var outer: PackedVector3Array = sample.get("outer", PackedVector3Array())
...
func _build_hollow_tube(samples: Array, tint: Color) -> ArrayMesh:
	...
	for s in samples:
		...
		var outer: PackedVector3Array = s.get("outer", PackedVector3Array())
		var inner := _inset_poly(outer, THICKNESS_FRAC)
	...
	for i in range(rings.size() - 1):
		...
		var ao: PackedVector3Array = a["outer"]
		var bo: PackedVector3Array = b["outer"]
		var ai: PackedVector3Array = a["inner"]
		var bi: PackedVector3Array = b["inner"]
		var n := mini(ao.size(), bo.size())
		if n < 3:
			continue
		if ao.size() != bo.size():
	...
	for i in range(rings.size()):
		_add_poly_border(st, rings[i]["outer"], cols[i])
	st.generate_normals()
	return st.commit()
...
func _inset_poly(outer: PackedVector3Array, frac: float) -> PackedVector3Array:
	...
	var t := clampf(1.0 - frac, 0.05, 0.95)
	for p in outer:
...
func _resample_closed(poly: PackedVector3Array, n: int) -> PackedVector3Array:
	...
	for k in range(n):
		var t := float(k) / float(n) * float(poly.size())
		var i0 := int(floor(t)) % poly.size()
		var i1 := (i0 + 1) % poly.size()
...
func _fallback_local_circle(center: Vector3, radius: float) -> PackedVector3Array:
	var out := PackedVector3Array()
	for k in range(FALLBACK_SIDES):
		var ang := TAU * float(k) / float(FALLBACK_SIDES)
...
func _team_tint(team_player: bool) -> Color:
	...
	if team_player:
		return Color(
			float(vis.get("booster_tint_player_r", 0.15)),
			float(vis.get("booster_tint_player_g", 0.55)),
			float(vis.get("booster_tint_player_b", 1.0)),
			float(vis.get("booster_tint_player_a", 1.0))
		)
	return Color(
		float(vis.get("booster_tint_enemy_r", 1.0)),
		float(vis.get("booster_tint_enemy_g", 0.12)),
		float(vis.get("booster_tint_enemy_b", 0.08)),
		float(vis.get("booster_tint_enemy_a", 1.0))
	)
...
static func _engine_locals_for(unit: Node3D) -> Array[Vector3]:
	...
	if out.is_empty() and unit.has_method("get_engine_local"):
		out.append(unit.call("get_engine_local") as Vector3)
	if out.is_empty():
