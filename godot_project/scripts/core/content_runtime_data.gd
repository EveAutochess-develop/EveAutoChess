...
const RUNTIME_DATA_ROOT := "user://content_runtime/data"
const RES_DATA_ROOT := "res://data"
...
const SEED_MANIFEST := "user://content_runtime/data/_seed_manifest.json"
...
static func ensure_seeded() -> Dictionary:
	var force := OS.has_feature("editor")
	var wrote := 0
	var skipped := 0
	var root_abs := ProjectSettings.globalize_path(RUNTIME_DATA_ROOT)
	DirAccess.make_dir_recursive_absolute(root_abs)
	var baseline: Array[String] = []
	for sub in EXPOSE_SUBDIRS:
		var r := _seed_dir(sub, force)
		wrote += int(r.get("wrote", 0))
		skipped += int(r.get("skipped", 0))
		for rel in r.get("baseline", []):
			baseline.append(str(rel))
	for fn in EXPOSE_ROOT_FILES:
		if _seed_file(fn, force):
			wrote += 1
		else:
			skipped += 1
		baseline.append(fn)
	var pruned := _prune_removed_baseline(baseline, force)
...
static func _prune_removed_baseline(baseline: Array[String], force: bool) -> int:
	...
	for rel in _read_seed_manifest():
		if not current.has(rel):
			stale[rel] = true
	if force:
		for sub in EXPOSE_SUBDIRS:
			for rel in _list_runtime_json(sub):
				if not current.has(rel):
					stale[rel] = true
	var n := 0
	for rel in stale.keys():
		var abs_path := ProjectSettings.globalize_path(runtime_path(str(rel)))
...
static func _list_runtime_json(rel_dir: String) -> Array[String]:
	...
	var dir := DirAccess.open(runtime_path(rel_dir))
	if dir == null:
		return out
	dir.list_dir_begin()
	var fn := dir.get_next()
...
static func _read_seed_manifest() -> Array[String]:
	...
	if not FileAccess.file_exists(SEED_MANIFEST):
		return out
	var data = JSON.parse_string(FileAccess.get_file_as_string(SEED_MANIFEST))
	if typeof(data) != TYPE_DICTIONARY:
		return out
	for rel in (data as Dictionary).get("files", []):
		out.append(str(rel))
	return out


static func _write_seed_manifest(baseline: Array[String]) -> void:
	var f := FileAccess.open(SEED_MANIFEST, FileAccess.WRITE)
	if f == null:
		push_warning("[ContentRuntimeData] cannot write %s" % SEED_MANIFEST)
		return
	var sorted := baseline.duplicate()
	sorted.sort()
...
static func _seed_dir(rel_dir: String, force: bool = false) -> Dictionary:
	...
	var skipped := 0
	var baseline: Array[String] = []
	var src_dir := res_path(rel_dir)
	var dir := DirAccess.open(src_dir)
	if dir == null:
		return {"wrote": 0, "skipped": 0, "baseline": baseline}
	var dst_abs := ProjectSettings.globalize_path(runtime_path(rel_dir))
...
static func _seed_file(rel: String, force: bool = false) -> bool:
	...
	var dst := runtime_path(rel)
	if not force and FileAccess.file_exists(dst):
		return false
	var src := res_path(rel)
	...
	if text.is_empty():
		...
		text = f.get_as_text()
		f.close()
	var parent := ProjectSettings.globalize_path(dst.get_base_dir())
...
static func load_json_prefer_runtime(rel: String) -> Dictionary:
	var runtime := runtime_path(rel)
	if FileAccess.file_exists(runtime):
		var parsed := _parse_json_file(runtime)
...
static func load_dir_overlay(rel_dir: String, on_item: Callable) -> void:
	var seen: Dictionary = {}
	_for_each_json(res_path(rel_dir), func(path: String, d: Dictionary):
		seen[path.get_file()] = true
		if on_item.is_valid():
			on_item.call(d, path, false)
	)
	_for_each_json(runtime_path(rel_dir), func(path: String, d: Dictionary):
		seen[path.get_file()] = true
		if on_item.is_valid():
		...
		if not dir.current_is_dir() and fn.ends_with(".json"):
			...
			if d.is_empty() and not _file_is_empty_object(full):
				...
				if dir_path.begins_with(RUNTIME_DATA_ROOT):
					var rel := full.trim_prefix(RUNTIME_DATA_ROOT).lstrip("/")
