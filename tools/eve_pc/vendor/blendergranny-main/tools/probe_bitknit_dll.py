#!/usr/bin/env python3
"""Probe Granny 2.11.8 BitKnitDecompress on raw section bytes.

Research-only helper. It compiles `tools/bitknit_dll_probe.c` into
`research_artifacts/` and runs it through Wine on Linux.
"""
...
DEFAULT_BUILD_DIR = Path("research_artifacts/bitknit_dll_probe")
...
def main() -> int:
    ...
    out_path = args.build_dir / f"{args.path.stem}.section{section.index}.dll.bin"
    raw_path.write_bytes(gr2.section_bytes(section))

    command = [str(exe), str(args.dll), str(raw_path), str(section.expanded_size), str(out_path)]
    ...
    print(
        f"{args.path}: section={section.index} size={len(data)} "
        f"sha256={hashlib.sha256(data).hexdigest()} out={out_path}"
    )
    return 0
...
def _build_probe(build_dir: Path) -> Path:
    ...
    exe = build_dir / "bitknit_dll_probe.exe"
    source = Path(__file__).with_name("bitknit_dll_probe.c")
    ...
    if exe.exists() and exe.stat().st_mtime >= source.stat().st_mtime:
        return exe
    subprocess.run(
        [compiler, "-O2", "-static", "-o", str(exe), str(source)],
        check=True,
    )
    return exe
